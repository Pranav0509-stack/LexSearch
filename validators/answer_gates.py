...
def validate(
    ...
    *,
    ...
    """Run gates over an answer.

    `mode` controls which gates apply:
      - "research" (default): all six gates (citation-grounded research mode).
      - "web": G1 (cite_present) + G2 (cite_resolves) + G3 (banned phrases) only.
        Web snippets aren't statute/case records, so G4–G6 don't apply.
      - "draft": G3 only (banned-phrase hygiene). Drafting mode skips retrieval
        entirely; there is nothing to ground against.
    """
    ...
    if mode == "draft":
        # Drafting: only banned phrases matter. Citations / grounding are N/A.
        passed = gates["no_banned_phrases"]
    elif mode == "web":
        # Web: must have a cite that resolves to a snippet, no hedge phrases.
        passed = (
            gates["cite_present"]
            and gates["cite_resolves"]
            and gates["no_banned_phrases"]
        )
    else:
        # Research: hard-fail if cite_present, cite_resolves, or scope_check fail.
