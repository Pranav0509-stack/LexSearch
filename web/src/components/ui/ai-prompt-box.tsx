...
import { ArrowUp, Paperclip, Square, X, StopCircle, Mic, Globe, BrainCog, FolderCode, Wand2 } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

// Utility function for className merging
const cn = (...classes: (string | undefined | null | false)[]) => classes.filter(Boolean).join(" ");

// Embedded CSS for minimal custom styles — injected via effect so SSR is safe
const styles = `
  *:focus-visible {
...
const VoiceRecorder: React.FC<VoiceRecorderProps> = ({
  ...
  React.useEffect(() => {
    if (isRecording) {
      ...
      // Defer setState off the effect body — keeps React 19's
      // "no setState during effect" rule happy.
      const stopped = time;
      queueMicrotask(() => {
  ...
  // Pre-computed bar heights. React 19's purity rule disallows Math.random()
  // anywhere in the render path (incl. useMemo), so we randomise once in an
  // effect and stash the result in state.
  const [barStyles, setBarStyles] = React.useState<
    { height: string; animationDuration: string }[]
  >(() => Array.from({ length: visualizerBars }, () => ({ height: "30%", animationDuration: "0.7s" })));
  React.useEffect(() => {
    // Defer the randomised state update to the next microtask so React 19's
    // "no setState in effect body" rule doesn't fire.
    queueMicrotask(() =>
      setBarStyles(
...
interface PromptInputBoxProps {
  onSend?: (message: string, files?: File[], opts?: { search?: boolean; think?: boolean; canvas?: boolean; agent?: boolean }) => void;
  isLoading?: boolean;
  placeholder?: string;
  className?: string;
}
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  ...
  const handleSubmit = () => {
    if (input.trim() || files.length > 0) {
      onSend(input, files, { search: showSearch, think: showThink, canvas: showCanvas, agent: showAgent });
      setInput("");
      setFiles([]);
      setFilePreviews({});
    }
    ...
    <>
      ...
      >
        ...
        <div className={cn("transition-all duration-300", isRecording ? "h-0 overflow-hidden opacity-0" : "opacity-100")}>
          <PromptInputTextarea
            placeholder={
              showAgent ? "Ask the agent — it will search, draft, and reason in steps…"
          ...
          >
            ...
            <div className="flex items-center">
              ...
              <button
                ...
                className={cn(
                  "rounded-full transition-all flex items-center gap-1 px-2 py-1 border h-8",
                  showAgent ? "bg-[#22C55E]/15 border-[#22C55E] text-[#22C55E]" : "bg-transparent border-transparent text-[#9CA3AF] hover:text-[#D1D5DB]"
                )}
                title="Agent — Gemini chains tools (search, retrieve, redline, translate) until it has the answer"
              >
                <div className="w-5 h-5 flex items-center justify-center flex-shrink-0">
                  <motion.div
                    animate={{ rotate: showAgent ? 360 : 0, scale: showAgent ? 1.1 : 1 }}
                    whileHover={{ rotate: showAgent ? 360 : 15, scale: 1.1, transition: { type: "spring", stiffness: 300, damping: 10 } }}
                    transition={{ type: "spring", stiffness: 260, damping: 25 }}
                  >
                    <Wand2 className={cn("w-4 h-4", showAgent ? "text-[#22C55E]" : "text-inherit")} />
                  </motion.div>
                </div>
                <AnimatePresence>
                  {showAgent && (
